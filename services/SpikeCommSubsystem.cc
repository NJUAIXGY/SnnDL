...
std::mutex SpikeCommSubsystem::s_route_cache_mtx_;
std::unordered_map<std::string, std::weak_ptr<const SpikeCommSubsystem::RouteMap>> SpikeCommSubsystem::s_route_cache_;
...
void SpikeCommSubsystem::configure(const SpikeCommRoutingConfig& routing_cfg,
                                   const SpikeCommGatingConfig& gating_cfg) {
    routing_cfg_ = routing_cfg;
    gating_cfg_ = gating_cfg;
...
void SpikeCommSubsystem::bindRuntime(const SpikeCommRuntimeConfig& rt) {
    ...
    num_neurons_ = rt.num_neurons;
    neurons_per_pe_cfg_ = rt.neurons_per_pe_cfg;
    if (rt.stat_fanout) stat_fanout_per_spike_ = rt.stat_fanout;
    if (rt.stat_routes_entries) stat_routes_entries_ = rt.stat_routes_entries;
}

void SpikeCommSubsystem::initRouting() {
    // Route table build + shared cache (only for weight-driven routing).
    if (routing_cfg_.routing_weight_driven) {
        const std::string cache_key = buildRouteCacheKey_(routing_cfg_);
        bool ok = false;
        ...
        if (!ok) {
            ...
            parseLayerMask_(routing_cfg_.route_layers_mask, allowed_layer_edges, allow_all_layers);

            RouteMap built_routes;
            if (routing_cfg_.mapping_mode == "edges_csv" && !routing_cfg_.mapping_edges_file.empty()) {
                ok = buildRoutesFromEdgesCSV_(routing_cfg_, log_, allowed_layer_edges, allow_all_layers, built_routes);
            } else {
                ok = buildWeightDrivenRoutesDense_(routing_cfg_, log_, allowed_layer_edges, allow_all_layers, built_routes);
            }
            ...
            if (!ok) {
                ...
                routing_cfg_.routing_weight_driven = false;
            } else {
                ...
                if (routing_cfg_.route_summary_enable && log_ && !route_summary_logged_) {
        ...
        if (routing_cfg_.route_summary_enable && log_) {
            logRoutingSummary_("init", routing_cfg_.routing_weight_driven ? (hit_cache ? "active(cache)" : "active") : "fallback_fixed");
        }
    } else {
        // Fixed routing: no route table is needed.
        routes_shared_.reset();
        routes_local_fallback_.clear();
    }

    // Configure route provider with the final routing mode.
    SnnRouteProvider::Config cfg{};
    cfg.routing_weight_driven = routing_cfg_.routing_weight_driven;
    cfg.log_weight_details = routing_cfg_.log_weight_details;
    cfg.num_neurons = num_neurons_;
    cfg.neurons_per_pe_cfg = neurons_per_pe_cfg_;
    cfg.node_id = node_id_;
    cfg.gating_event_mode = gating_cfg_.gating_event_mode;
    cfg.gating_scope_inputs_only = gating_cfg_.gating_scope_inputs_only;
    cfg.gating_cache = &gating_cache_;
    cfg.out = log_;
    cfg.stat_fanout = stat_fanout_per_spike_;
    route_provider_.configure(cfg, routes_shared_, &routes_local_fallback_);
...
void SpikeCommSubsystem::emitCommon_(uint32_t source_global, uint32_t source_local, uint64_t now_cycle) {
    if (!transport_ || !route_provider_ready_) return;
    std::vector<SnnRouteProvider::FanoutEntry> fanouts;
    bool applied_gating = false;
    route_provider_.computeFanout(source_global, source_local, now_cycle, fanouts, applied_gating);
...
void SpikeCommSubsystem::applyGatingDecision(uint32_t src_global,
                                             ...
                                             uint64_t ttl_cycles) {
    if (!gating_cfg_.gating_event_mode) return;
    GatingEntry e;
    e.dest_pes = dest_pes;
    e.expire_cycle = current_cycle + (ttl_cycles ? ttl_cycles : gating_cfg_.gating_ttl_cycles);
...
void SpikeCommSubsystem::logRoutingSummary_(const char* phase, const char* reason) const {
    if (!(routing_cfg_.route_summary_enable && log_)) return;
