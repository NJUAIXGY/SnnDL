...
//
// SnnNetworkAdapter.cc: legacy (archived) implementation file
//
...
#endif // legacy SpikeEvent-based implementation (disabled)

// ===== NetworkEventConverter（通用事件版本）=====
    ...
    : SnnInterface(id, params)
{
    ...
    const std::string topology_str = params.find<std::string>("topology_type", "mesh2d");
    topology_type = parseTopologyType(topology_str);
    topology_shape = params.find<std::string>("topology_shape", "4x4");
    ...
    if (enable_merlin_router) {
        ...
        if (router) {
            router->setNotifyOnReceive(new SST::Interfaces::SimpleNetwork::Handler2<SnnNetworkAdapter,&SnnNetworkAdapter::handleIncoming>(this));
            router->setNotifyOnSend(new SST::Interfaces::SimpleNetwork::Handler2<SnnNetworkAdapter,&SnnNetworkAdapter::spaceAvailable>(this));
        }
    }
}
...
void SnnNetworkAdapter::setNodeId(uint32_t id)
{
    node_id = id;
}
...
{
    std::stringstream ss;
    ss << "SnnNetworkAdapter[node=" << node_id << "]";
    ss << " pending=" << pending_sends_.size();
    return ss.str();
}
...
bool SnnNetworkAdapter::spaceAvailable(int /*vn*/)
{
    ...
    while (!pending_sends_.empty()) {
        ...
        if (!ps.payload) { pending_sends_.pop(); continue; }
        auto* req = createNetworkRequest(ps.payload, ps.dest_node, /*route_port=*/0);
        bool sent = false;
...
void SnnNetworkAdapter::initializeTopologyHandler()
{
    ...
    if (topology_type == TopologyType::TORUS_2D) {
        topology_handler = std::make_unique<Torus2DHandler>();
    } else {
        topology_handler = std::make_unique<Mesh2DHandler>();
    }
