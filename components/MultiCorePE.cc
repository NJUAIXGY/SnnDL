...
MultiCorePE::MultiCorePE(ComponentId_t id, Params& params)
    ...
    if (pulse_enable_) {
        ...
        if (!pulse_cfg.observe_only &&
            ...
            output_->fatal(CALL_INFO, -1,
                           "PULSE actual ingress requires pulse_ingress_enable=1 and pulse_ingress_entries>0 "
                           "(node=%d ingress_enable=%d ingress_entries=%u)\n",
                           node_id_, pulse_cfg.ingress_enable ? 1 : 0,
                           pulse_cfg.ingress_entries);
        }
        if (!pulse_cfg.agenda_observe_only) {
            output_->fatal(CALL_INFO, -1,
                           "pulse_agenda_observe_only=0 is unsupported; no actual agenda service is registered\n");
        }
        // pulse_descriptor_actual_enable controls the core-side WMS service.
        // PeSharedCoreFabric::Config defaults its unsupported descriptor
        // service capability to false.
