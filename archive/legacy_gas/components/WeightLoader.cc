...
bool WeightLoader::bindBcsrSourceContract_(int core, const std::string& path) {
    // Dense and synthetic fill paths have no file-side topology to bind.
    if (!raw_mode_ || !bcsr_enable_ || path.empty()) return true;

    BcsrFileSource source;
    std::string source_error;
    if (!loadBcsrFileSource(path, rows_per_core_, source, &source_error,
                            /*compute_content_fingerprint=*/true)) {
        // Keep the loader's legacy raw-file behavior intact.  The route side
        // will reject a BCSR source without valid metadata when it is enabled.
        if (output_ && verbose_ >= 2) {
            output_->verbose(CALL_INFO, 2, 0,
                             "[WL-source-contract] skip core=%d path=%s: %s\n",
                             core, path.c_str(), source_error.c_str());
        }
        return true;
    }

    const std::string slot =
        "pe=" + std::to_string(node_id_) + "/core=" + std::to_string(core);
    std::string contract_error;
    if (!bindBcsrSourceContract(slot, path, source.identity(), "weight-loader",
                                &contract_error)) {
        output_->fatal(CALL_INFO, -1,
                       "WeightLoader BCSR source contract violation: %s\n",
...
bool WeightLoader::loadPerCoreFiles(const std::string& tmpl, const std::string& fmt) {
    ...
    for (int core = 0; core < num_cores_; ++core) {
        ...
        if (!bindBcsrSourceContract_(core, path)) return false;
        if (raw_mode_) {
            std::vector<uint8_t> raw;
