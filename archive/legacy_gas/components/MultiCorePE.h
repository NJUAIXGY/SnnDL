...
class MultiCorePE : public SST::Component,
    ...
    class CapabilityAdapter final : public IPeCapabilityProvider,
                                    public IPeDiagnosticsSink,
                                    public IPeRuntimeConfig,
    ...
    public:
        explicit CapabilityAdapter(MultiCorePE& owner) : owner_(owner) {}

        IPeDiagnosticsSink* peDiagnosticsSink() override;
        IPeRuntimeConfig* peRuntimeConfig() override;
        IDmaSchedulerProvider* dmaSchedulerProvider() override;
        ILocalStorageProvider* localStorageProvider() override;
        IPePodSharedMetadataProvider* podMetadataProvider() override;
        IPeWeightObjectPlaneProvider* weightObjectPlaneProvider() override;

        SST::Statistics::Statistic<uint64_t>*
        getComputeActiveCyclesTotalStatistic() const override;
        void recordStepGasStat(uint32_t seq, const GasStatEvent& st) override;
        void recordCoreStepGasStat(int core_id, uint32_t seq,
                                   const GasStatEvent& st) override;
        void recordStepApplyScatter(uint32_t seq,
                                    ...
                                    uint64_t spilled_bytes) override;
        void recordCoreStepApplyScatter(int core_id, uint32_t seq,
                                        uint64_t acc_updates,
                                        uint64_t posts_touched,
                                        uint64_t spikes_emitted,
                                        uint64_t hwm_bytes,
        ...
        void accumulateRiscvSnnRuntimeStats(uint64_t workload_selected,
                                            ...
                                            uint64_t backend_runtime_bridge_provider_bound) override;

        PeDmaScheduler* dmaScheduler() override;
        LocalStorageHierarchyController* localStorageHierarchy() override;
        PodMetadataObjectPlane* pePodMetadataObjectPlane() override;
        PodOwnerServiceTable* pePodOwnerServiceTable() override;
        PeLocalServiceObjectTable* peLocalServiceObjectTable() override;
        PeWeightObjectPlane* peWeightObjectPlane() override;
        bool peInternalCpeEnabledConfig() const override;
        bool peInternalPodEnabledConfig() const override;
        bool peInternalPodMetadataEnabledConfig() const override;
        bool peInternalPodOwnerEnabledConfig() const override;
        uint32_t peInternalPodCountConfig() const override;
        uint32_t peInternalPodSizeConfig() const override;
...
public:
    ...
    PeDmaScheduler* dmaScheduler();
    LocalStorageHierarchyController* localStorageHierarchy();
    const LocalStorageHierarchyController* localStorageHierarchy() const;
    PodMetadataObjectPlane* pePodMetadataObjectPlane();
    PodOwnerServiceTable* pePodOwnerServiceTable();
    PeLocalServiceObjectTable* peLocalServiceObjectTable();
    PeWeightObjectPlane* peWeightObjectPlane();
    bool peInternalCpeEnabledConfig() const;
    bool peInternalPodEnabledConfig() const;
    bool peInternalPodMetadataEnabledConfig() const;
    bool peInternalPodOwnerEnabledConfig() const;
    uint32_t peInternalPodCountConfig() const;
    uint32_t peInternalPodSizeConfig() const;
...
private:
    ...
    CapabilityAdapter capabilities_;
    ...
    std::unique_ptr<PeOptionalServices> optional_services_;
