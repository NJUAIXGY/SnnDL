...
std::unique_ptr<ISynapseRoute> makeSynapseRoute_() {
    // The native SNN workload is intentionally 2D-only.  3D route providers
    // remain available through libSnnDLResearch and are not selected here.
...
void SnnWorkload::bindRuntime(const Runtime& rt) {
    ...
    if (synapse_route_) {
        ...
        // The native workload has no 3D statistics sink.  Keep the extension
        // hook initialized with an empty sink for ABI compatibility.
        synapse_route_->bindRouteRuntimeStats({});
    }
    if (spike_comm_) {
...
void SnnWorkload::ensureSpikeCommConfigured_() {
    ...
    if (!synapse_route_) synapse_route_ = makeSynapseRoute_();
    ...
    cfg.multicast_block_d = 1u;
    cfg.multicast_die_local_only = params_->find<int>("multicast_die_local_only", 0) != 0;
    // The core SNN route is 2D.  Any 3D target synthesis belongs to the
    // explicit research library and is never inferred from mesh_shape here.
    cfg.route3d_native_targets = false;
...
void SnnWorkload::getStatistics(std::map<std::string, uint64_t>& stats) const {
    ...
    // Semantic region and 3D route statistics belong to optional workloads and
    // the research library.  The native SNN workload reports only its own
    // 2D/GAS/BCSR counters here.
    if (compute_core_) {
