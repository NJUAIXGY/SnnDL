...
#include "snn/profiling/SnnProfiler.h"  // 轻量级性能分析（条件编译）
...
class SnnPESubComponent : public CoreShellAPI,
...
public:
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"tensor_collective_overlap_with_compute", "Tensor workload: allow snn/compute/collective overlap (0/1)", "1"},
        ...
        {"total_cores", "Total number of cores in the PE (overridden by parent PE)", "1"},
        ...
        {"total_nodes", "Total number of PEs/nodes in the system (overridden by parent PE)", "1"},
        ...
        {"synapse_weight_mode", "Synapse weight sourcing mode (fixed): bcsr_gas", "bcsr_gas"},
    ...
    void setCapabilityProvider(IPeCapabilityProvider* provider) override;
    bool applyPlatformConfig(const CorePlatformConfig& config,
...
private:
    std::unique_ptr<IWeightReader> takeWeightReader_();
    void recordNeuronFires_(const std::vector<uint32_t>& neuron_indices);
    ...
    bool read_force_single_ = false; // 当为真时，强制按单元素读取（req_size=4B），用于定位对齐/切片问题
        ...
        std::unique_ptr<ISnnAccelRuntimeServices> riscv_snn_runtime_bridge_;
        ...
        IPeCapabilityProvider* pe_capabilities_ = nullptr;
        IPeDiagnosticsSink* pe_diagnostics_ = nullptr;
        IPeRuntimeConfig* pe_runtime_config_ = nullptr;
    Output* output_;
