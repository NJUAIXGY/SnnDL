...
#include "api/IPeCapabilityProvider.h"
...
#include "ISnnAccelRuntimeServices.h"
...
void SnnPESubComponent::reportApplyScatterThunk_(void* ctx,
    ...
    if (auto* pe = core->pe_diagnostics_) {
...
SnnPESubComponent::SnnPESubComponent(ComponentId_t id, Params& params)
    ...
    const bool kSentinelOn = cfg.identity.sentinel_enable;
    ...
    total_cores_ = cfg.identity.total_cores;
    global_neuron_base_ = cfg.identity.global_neuron_base;
    num_neurons_ = cfg.identity.num_neurons;
    base_addr_ = cfg.identity.base_addr;
    node_id_ = cfg.identity.node_id;
    verbose_ = cfg.identity.verbose;
    enable_extended_diagnostics_ = cfg.diagnostics.enable_extended_diagnostics;
    total_nodes_cfg_ = cfg.identity.total_nodes;
    if (output_) {
    ...
    const WorkloadKind workload_kind = workloadKindFromString(cfg.workload.workload_impl);
    ...
    // Phase6：核心 SNN 走窄工厂；其它 workload 由扩展注册表提供。
    // 保留 CoreWorkloadFactory 作为外部兼容入口，但平台装配器不再依赖
    // 一个同时知道所有 workload 实现的总工厂。
    {
        const char* name = workloadKindName(workload_kind);
        if (!workload_ && workload_kind == WorkloadKind::Snn) {
            workload_ = createCoreSnnWorkload();
        }
        if (!workload_) {
            workload_ = createOptionalWorkloadByName(std::string(name));
        }
        if (!workload_) {
            if (output_) {
                output_->fatal(CALL_INFO, -1,
                               "no core or optional workload factory is registered for \"%s\"\n",
                               name);
            }
            return;
        }
    ...
    if (workload_kind == WorkloadKind::RiscvSnn &&
        ...
        if (!riscv_snn_runtime_bridge_) {
            riscv_snn_runtime_bridge_ =
                createOptionalRuntimeServiceByName("riscv_snn_runtime_bridge");
        }
        if (riscv_snn_runtime_bridge_) {
            ...
            output_->fatal(
                CALL_INFO, -1,
                "riscv_snn_backend_name=runtime_bridge requires the explicitly loaded "
                "SnnDL Research runtime-service provider\n");
        }
    ...
    enable_weight_fetch_ = cfg.workload.enable_weight_fetch;
    workload_spike_input_enable_ = cfg.workload.workload_spike_input_enable;
...
#endif
    write_weights_on_init_ = cfg.workload.write_weights_on_init;
    memory_warmup_cycles_ = cfg.workload.memory_warmup_cycles;
    init_default_weight_ = cfg.workload.init_default_weight;
    readresp_zero_fallback_ = cfg.workload.readresp_zero_fallback;
    max_outstanding_requests_ = cfg.workload.max_outstanding_requests;
    const uint32_t max_cache_entries = cfg.storage.max_cache_entries;
    const bool use_clock_weight_cache = cfg.storage.use_clock_weight_cache;
    const bool disable_weight_cache = cfg.storage.disable_weight_cache;
    {
    ...
    use_event_weight_fallback_ = cfg.storage.use_event_weight_fallback;
    event_weight_fallback_warned_ = false;
    route_summary_enable_ = cfg.communication.route_summary_enable;
    merge_read_cacheline_ = cfg.communication.merge_read_cacheline;
    merge_read_row_ = cfg.communication.merge_read_row;
    gas_enable_ = cfg.gas.gas_enable; // 默认关闭
    gas_window_mode_ = cfg.gas.gas_window_mode; // 当为true时采用GatherBufferIF的window驱动
    // 已弃用：保留参数解析以兼容旧配置，但运行期强制忽略
    bool manual_drive_param = cfg.gas.gas_manual_window_drive_requested;
    if (manual_drive_param && output_ && output_->getVerboseLevel() >= 2) {
        output_->verbose(CALL_INFO, 2, 0,
            "[diag-gas-config] core=%d gas_manual_window_drive 已弃用，仍将使用自动窗口驱动\n",
            core_id_);
    }
    merge_read_auto_ = cfg.communication.merge_read_auto; // default off
    line_size_bytes_ = cfg.communication.line_size_bytes;
    byte_exact_verify_enable_ = cfg.verification.byte_exact_verify_enable;
    byte_exact_verify_mode_ = cfg.verification.byte_exact_verify_mode;
    byte_exact_verify_row_scale_ = cfg.verification.byte_exact_verify_row_scale;
    byte_exact_verify_max_mismatch_ = cfg.verification.byte_exact_verify_max_mismatch;
    bcsr_semantic_verify_enable_ = cfg.verification.bcsr_semantic_verify_enable;
    bcsr_semantic_verify_max_edges_ = cfg.verification.bcsr_semantic_verify_max_edges;
    bcsr_semantic_verify_max_mismatch_ = cfg.verification.bcsr_semantic_verify_max_mismatch;
    bcsr_semantic_verify_abs_tol_ = cfg.verification.bcsr_semantic_verify_abs_tol;
    bcsr_semantic_verify_rel_tol_ = cfg.verification.bcsr_semantic_verify_rel_tol;
    loader_done_key_ = cfg.storage.loader_done_key;
    ...
    window_read_enable_ = cfg.gas.window_read_enable;
    window_read_debug_ = cfg.gas.window_read_debug;
    scatter_diag_limit_ = cfg.gas.scatter_diag_limit;
    scatter_diag_count_ = 0;
    if (impl_ && impl_->gas_ctrl_) impl_->gas_ctrl_->setDebug(window_read_debug_, enable_extended_diagnostics_);
    window_read_budget_ = cfg.gas.window_read_budget;
    windowStateConfigure_();
    read_force_single_ = cfg.storage.read_force_single;
    // 边集合容量上限（极端保护）
    edge_collector_max_capacity_ = static_cast<size_t>(cfg.gas.edge_collector_max_capacity);
    ...
    // 全网读取扩展参数
    weights_cols_ = cfg.communication.weights_cols;
    synapse_weight_mode_ = "bcsr_gas";
    std::string index_mode_str = cfg.communication.index_mode;
    // 神经元状态布局参数已下沉到 compute core（控制层不再持有）
    verify_routing_weights_ = cfg.communication.verify_routing_weights;
    ...
    dense_layout_mode_ = toLowerCopy(cfg.storage.dense_layout_mode);
    if (dense_layout_mode_.empty()) dense_layout_mode_ = "row_major";
    dense_phys_dram_row_bytes_ = cfg.storage.dense_phys_dram_row_bytes;
    ...
    enable_detailed_map_log_ = cfg.diagnostics.enable_detailed_map_log;
    log_weight_details_ = cfg.diagnostics.log_weight_details;
    loader_barrier_cycles_ = cfg.storage.loader_barrier_cycles;
    // BCSR 参数
    bcsr_layout_.rows = num_neurons_;
    bcsr_layout_.cols = cfg.routing.bcsr_cols;
    bcsr_layout_.block_rows = cfg.routing.bcsr_block_rows;
    bcsr_layout_.block_cols = cfg.routing.bcsr_block_cols;
    bcsr_layout_.idx_bytes = cfg.routing.bcsr_idx_bytes;
    bcsr_layout_.val_bytes = cfg.routing.bcsr_val_bytes;
    bcsr_layout_.rowptr_offset = cfg.routing.bcsr_rowptr_offset;
    bcsr_layout_.colidx_offset = cfg.routing.bcsr_colidx_offset;
    bcsr_layout_.blockdata_offset = cfg.routing.bcsr_blockdata_offset;
    bcsr_layout_.blockids_offset = cfg.routing.bcsr_blockids_offset;
    bcsr_layout_.layout_mode = cfg.routing.bcsr_layout_mode;
    bcsr_layout_.colidx_row_stride_bytes = cfg.routing.bcsr_colidx_row_stride_bytes;
    bcsr_layout_.blockdata_row_stride_bytes = cfg.routing.bcsr_blockdata_row_stride_bytes;
    bcsr_layout_.blockids_row_stride_bytes = cfg.routing.bcsr_blockids_row_stride_bytes;
    bcsr_layout_.per_core_stride = cfg.routing.per_core_stride;
    ...
    bcsr_row_index_cache_cap_ = cfg.routing.bcsr_row_index_cache_cap;
    bcsr_block_cache_cap_ = cfg.routing.bcsr_block_cache_cap;
    // GAS Apply/Scatter Phase‑1
    apply_acc_enable_ = cfg.gas.apply_acc_enable;
    acc_hwm_bytes_cfg_ = cfg.gas.acc_high_watermark_bytes;
    acc_spill_enable_cfg_ = cfg.gas.acc_spill_enable;
    stage_events_csv_ = cfg.gas.stage_events_csv;
    if (impl_ && impl_->gas_ctrl_) impl_->gas_ctrl_->setStageEventsCsv(stage_events_csv_);
    // aosoa_block_rows 默认推导已转移至 compute core
    // CSR 参数已移除
    bcsr_prefetch_all_ = cfg.routing.bcsr_prefetch_all;
    // 权重验证开关（具体采样/文件配置由 compute core 解析并执行）
    verify_weights_ = cfg.routing.verify_weights;
    bcsr_force_file_read_ = cfg.routing.bcsr_force_file_read;
    bcsr_rowptr_file_fallback_enable_ = cfg.routing.bcsr_rowptr_file_fallback_enable;
    quiet_finish_logs_ = cfg.diagnostics.quiet_finish_logs;
    record_edge_apply_enable_ = cfg.gas.record_edge_apply_enable;
    record_edge_idle_enable_ = cfg.gas.record_edge_idle_enable;
    ...
    record_edge_scatter_enable_ = cfg.gas.record_edge_scatter_enable;

    // ---- Profiling init (optional; minimal overhead when disabled) ----
#ifdef SNNDL_ENABLE_PROFILING
    profiler_enabled_ = cfg.diagnostics.profiler_enable;
    profiler_csv_prefix_ = cfg.diagnostics.profiler_csv_prefix;
...
#endif
    // 默认启用致密累加器（与头文件参数表一致）
    acc_dense_enable_cfg_ = cfg.gas.apply_dense_acc_enable;
    acc_shadow_verify_enable_cfg_ = cfg.gas.acc_shadow_verify_enable;
    ...
    weights_template_ = cfg.workload.weights_template;
    // neurons_per_pe：默认按 total_cores*num_neurons 推导；但允许脚本显式覆盖（保持兼容）
    uint32_t np_from_params = cfg.workload.neurons_per_pe;
    ...
    weights_file_path_ = cfg.workload.weights_file;
...
SnnPESubComponent::~SnnPESubComponent() {
    ...
    pe_capabilities_ = nullptr;
    pe_diagnostics_ = nullptr;
    pe_runtime_config_ = nullptr;
...
void SnnPESubComponent::setParentInterface(IPeAggregation* parent) {
    ...
    pe_capabilities_ = dynamic_cast<IPeCapabilityProvider*>(parent);
    if (pe_capabilities_) {
        pe_diagnostics_ = pe_capabilities_->peDiagnosticsSink();
        pe_runtime_config_ = pe_capabilities_->peRuntimeConfig();
    } else {
        // Compatibility path for older PE parents that implemented the
        // optional interfaces directly.
        pe_diagnostics_ = dynamic_cast<IPeDiagnosticsSink*>(parent);
        pe_runtime_config_ = dynamic_cast<IPeRuntimeConfig*>(parent);
    }
...
void SnnPESubComponent::setCapabilityProvider(IPeCapabilityProvider* provider) {
    pe_capabilities_ = provider;
    pe_diagnostics_ = provider ? provider->peDiagnosticsSink() : nullptr;
    pe_runtime_config_ = provider ? provider->peRuntimeConfig() : nullptr;
    refreshSharedWeightObjectPlaneBinding_();
    bindWorkloadRuntime_();
}
...
void SnnPESubComponent::refreshSharedWeightObjectPlaneBinding_() {
    ...
    auto* provider = pe_capabilities_
        ? pe_capabilities_->weightObjectPlaneProvider()
        : dynamic_cast<IPeWeightObjectPlaneProvider*>(parent_pe_cached_);
    auto* plane = provider ? provider->peWeightObjectPlane() : nullptr;
    auto* pod_provider = pe_capabilities_
        ? pe_capabilities_->podMetadataProvider()
        : dynamic_cast<IPePodSharedMetadataProvider*>(parent_pe_cached_);
    auto* metadata_plane =
    ...
    auto* runtime_cfg = pe_runtime_config_;
    if (runtime_cfg) {
        const uint32_t pod_count =
            std::max<uint32_t>(1u, runtime_cfg->peInternalPodCountConfig());
        const uint32_t pod_size =
            std::max<uint32_t>(1u, runtime_cfg->peInternalPodSizeConfig());
        const uint32_t pod_id =
            std::min<uint32_t>(
                static_cast<uint32_t>(core_id_) / pod_size,
                pod_count - 1u);
        weight_mem_subsystem_->setPeInternalPodRuntimeConfig(
            runtime_cfg->peInternalCpeEnabledConfig(),
            runtime_cfg->peInternalPodEnabledConfig(),
            runtime_cfg->peInternalPodMetadataEnabledConfig(),
            runtime_cfg->peInternalPodOwnerEnabledConfig(),
            pod_id,
            pod_count,
            pod_size,
            static_cast<uint32_t>(core_id_));
