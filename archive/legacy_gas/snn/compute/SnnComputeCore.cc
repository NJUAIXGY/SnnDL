...
struct DefaultSnnComputeCore::StateSramRuntime {
    VirtualSramLayout layout{};
    BankedSramModel model{};
};

DefaultSnnComputeCore::~DefaultSnnComputeCore() = default;
...
void DefaultSnnComputeCore::configure(const ComputeCoreContext& ctx, const SST::Params& params) {
    ...
    state_sram_enable_ = params.find<int>("state_sram_enable", 0) != 0;
    {
        ...
        if (!state_sram_) state_sram_ = std::make_unique<StateSramRuntime>();
        state_sram_->layout.configure(lcfg);
        ...
        state_sram_->model.configure(scfg);
    }
...
void DefaultSnnComputeCore::initCoreEngineState_() {
    ...
    if (state_sram_enable_) {
        ...
        state_sram_->model.noteResidentBytes(resident_bytes);
    }
}
...
void DefaultSnnComputeCore::onFinish() {
    if (state_sram_enable_) {
        state_sram_->model.onClockTick(state_sram_current_cycle_ + 1);
    }
}
...
void DefaultSnnComputeCore::onClockTick(uint64_t now_cycle) {
    ...
    if (state_sram_enable_) {
        state_sram_->model.onClockTick(now_cycle);
        state_sram_stall_budget_ = satAddU64Local(
            state_sram_stall_budget_,
            state_sram_->model.consumeLastCyclePredictedExtraCycles());
...
void DefaultSnnComputeCore::noteStateSramRead_(uint64_t addr, uint64_t bytes) {
    ...
    state_sram_->model.noteRead(state_sram_current_cycle_, addr, bytes);
}

void DefaultSnnComputeCore::noteStateSramWrite_(uint64_t addr, uint64_t bytes) {
    if (!state_sram_enable_) return;
    state_sram_->model.noteWrite(state_sram_current_cycle_, addr, bytes);
}
...
void DefaultSnnComputeCore::applyPendingDeltas_() {
    ...
    for (uint32_t post_local : pending_dv_list_) {
        ...
        if (state_sram_enable_) {
            noteStateSramRead_(state_sram_->layout.stateVmemAddr(post_local), sizeof(float));
            noteStateSramWrite_(state_sram_->layout.stateVmemAddr(post_local), sizeof(float));
        }
...
void DefaultSnnComputeCore::updateNeuronStates() {
    ...
    if (state_sram_enable_ && num_neurons_ > 0) {
        for (uint32_t i = 0; i < num_neurons_; ++i) {
            noteStateSramRead_(state_sram_->layout.stateVmemAddr(i), sizeof(float));
            noteStateSramRead_(state_sram_->layout.stateRefracAddr(i), sizeof(uint32_t));
            noteStateSramWrite_(state_sram_->layout.stateVmemAddr(i), sizeof(float));
            noteStateSramWrite_(state_sram_->layout.stateRefracAddr(i), sizeof(uint32_t));
        }
    }
    if (neuron_model_) {
        if (use_soa_state_) {
...
bool DefaultSnnComputeCore::fire(uint32_t idx, uint64_t now_cycles, INeuronModel* model,
    ...
    if (state_sram_enable_ && idx < num_neurons_) {
        noteStateSramRead_(state_sram_->layout.stateVmemAddr(idx), sizeof(float));
        noteStateSramRead_(state_sram_->layout.stateRefracAddr(idx), sizeof(uint32_t));
        noteStateSramRead_(state_sram_->layout.stateLastSpikeAddr(idx), sizeof(uint64_t));
    ...
    if (state_sram_enable_ && idx < num_neurons_) {
        ...
        if (fired || state_vmem_after != state_vmem_before) {
            noteStateSramWrite_(state_sram_->layout.stateVmemAddr(idx), sizeof(float));
        }
        if (fired || state_refrac_after != state_refrac_before) {
            noteStateSramWrite_(state_sram_->layout.stateRefracAddr(idx), sizeof(uint32_t));
        }
        if (fired || state_last_spike_after != state_last_spike_before) {
            noteStateSramWrite_(state_sram_->layout.stateLastSpikeAddr(idx), sizeof(uint64_t));
        }
    }
...
void DefaultSnnComputeCore::getStatistics(std::map<std::string, uint64_t>& out) const {
    ...
    const auto& sram_stats = state_sram_->model.stats();
