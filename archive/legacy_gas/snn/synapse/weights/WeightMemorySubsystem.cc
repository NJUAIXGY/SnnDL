...
void WeightMemorySubsystem::assertBcsrSourceContract_() {
    if (!orch_.use_bcsr || orch_.weights_template.empty()) return;

    const std::string path =
        resolveWeightsTemplatePath(orch_.weights_template, orch_.node_id, orch_.core_id);
    if (path.empty()) return;

    BcsrFileSource source;
    std::string source_error;
    if (!loadBcsrFileSource(path, orch_.num_neurons, source, &source_error,
                            /*compute_content_fingerprint=*/true)) {
        // Route construction owns the hard failure for malformed/missing
        // BCSR metadata.  Preserve the existing memory-side fallback policy.
        return;
    }

    const std::string slot =
        "pe=" + std::to_string(orch_.node_id) + "/core=" + std::to_string(orch_.core_id);
    std::string contract_error;
    if (!bindBcsrSourceContract(slot, path, source.identity(), "weight-memory",
                                &contract_error)) {
    ...
    if (meta.bcsr_kind == 1) {
        ...
        if (orch_.bcsr_mgr) {
            ...
            if (!ok && orch_.bcsr_rowptr_file_fallback_enable && !orch_.weights_template.empty()) {
                ...
                BcsrFileSource source;
                const bool source_ok = !path.empty() &&
                    loadBcsrFileSource(path, orch_.num_neurons, source, nullptr);
                if (source_ok && orch_.base_addr <= orch_.bcsr_mgr->rowptrAddr()) {
                    // Use the canonical metadata offset, not an independently
                    // interpreted copy of the JSON contract.
                    const uint64_t rp_off = source.meta.rowptr_offset;
                    ...
                    if (ok && !bcsr_rowidx_file_preloaded_ && mode == "all_rows" && orch_.bcsr_mgr->rowIndexCacheCapacity() > 0) {
                        ...
                        const uint64_t ci_base_addr = orch_.base_addr + source.meta.colidx_offset;
                        if (orch_.base_addr <= ci_base_addr) {
                            ...
                            const uint64_t ci_off = source.meta.colidx_offset;
