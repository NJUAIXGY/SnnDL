...
//   1) platform/memory/StandardMemAccess（纯数据面 read/write 回调）
//   2) api/IGasStageSink（GAS 控制面 stage/stat 事件，纯数据结构）
// - 目标：platform/core/** 彻底不出现 StandardMem::（包括 .cc）
//
...
public:
    ...
    // Handle a StandardMem response (opaque pointer to avoid leaking stdMem types into platform/core/**).
