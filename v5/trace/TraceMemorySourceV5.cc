...
TraceMemorySourceV5::TraceMemorySourceV5(SST::ComponentId_t id, SST::Params& params)
    ...
    if (trace_file_.empty()) out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 requires trace_file\n");
    trace_stream_.open(trace_file_);
    if (!trace_stream_.good()) out_.fatal(CALL_INFO, -1, "cannot open trace_file=%s\n", trace_file_.c_str());
    ...
    if (!memory_) out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 requires StandardMem slot 'memory'\n");
    if (external_control_) {
        ...
        if (!command_link_ || !status_link_) {
            out_.fatal(CALL_INFO, -1, "externally controlled TraceMemorySourceV5 requires command/status links\n");
        }
    }
...
bool TraceMemorySourceV5::readNext_(Record& result) {
    ...
    while (std::getline(trace_stream_, line)) {
        ...
        if (line.size() > max_line_bytes_) {
            out_.fatal(CALL_INFO, -1, "trace record exceeds max_line_bytes=%" PRIu64 "\n", max_line_bytes_);
        }
        try {
            const auto value = json::parse(line);
            if (!value.is_object() || value.value("schema_version", std::string()) != "snndl-materialized-request/v1")
                out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 requires snndl-materialized-request/v1 records\n");
            if (value.value("operation", std::string()) != "read")
                out_.fatal(CALL_INFO, -1, "TRACE-1 only supports read operations\n");
            ...
            if (result.request_id.empty() || result.access_id.empty() || result.request_class.empty() ||
                ...
                out_.fatal(CALL_INFO, -1, "invalid materialized request identity or size\n");
            if (!seen_request_ids_.insert(result.request_id).second)
                out_.fatal(CALL_INFO, -1, "duplicate materialized request_id=%s\n", result.request_id.c_str());
            if (have_sequence_ && result.stream_sequence <= last_stream_sequence_)
                out_.fatal(CALL_INFO, -1, "stream_sequence is not strictly increasing for source=%u\n", source_id_);
            have_sequence_ = true;
            last_stream_sequence_ = result.stream_sequence;
            ++records_;
            return true;
        } catch (const std::exception& error) {
            out_.fatal(CALL_INFO, -1, "invalid trace record: %s\n", error.what());
        }
    }
    return false;
}
...
void TraceMemorySourceV5::handleControl_(SST::Event* event) {
    ...
    if (!control) {
        delete event;
        out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 received an unexpected control event\n");
    }
    if (control->source_id != coordinator_source_id_) {
        delete control;
        out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 received a command for source=%u\n", control->source_id);
    }
    switch (control->operation) {
    case TraceControlOp::Start:
        if (started_ || finished_) {
            delete control;
            out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 received duplicate Start\n");
        }
        started_ = true;
        break;
    case TraceControlOp::Abort:
        delete control;
        out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 aborted by replay coordinator\n");
        break;
    }
    delete control;
}
...
void TraceMemorySourceV5::handleMemory_(SST::Interfaces::StandardMem::Request* request) {
    ...
    if (!response) {
        delete request;
        out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 received non-read response\n");
    }
    ...
    if (found == outstanding_.end()) {
        delete response;
        out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 received unknown response id=%" PRIu64 "\n", runtime_id);
    }
    const auto record = found->second.record;
    if (response->pAddr != record.address || response->size != record.bytes) {
        delete response;
        out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 response address/size mismatch\n");
    }
...
void TraceMemorySourceV5::finish() {
    ...
    if (!finished_ || !eligible_.empty() || !outstanding_.empty() || records_ != injected_ || completed_ != injected_)
        out_.fatal(CALL_INFO, -1, "TraceMemorySourceV5 finished before replay drained\n");
}
