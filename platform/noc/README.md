...
2) `workload=snn` 调用 `snn/synapse/route/SpikeCommSubsystem`；
3) `SpikeCommSubsystem` 通过 `api/ISpikeTransport` 发出 spike（常见实现是 `api/NocSpikeTransport`；也可使用 `snn/synapse/route/SpikePacketTransport`）；
4) `ISpikeTransport` → `INocTransport::sendFromCore(...)`；
5) `NocSubsystem` 根据 packet 头（`dst_node/dst_endpoint`）做本 PE 内投递或外发：
   - 本 PE：`deliver_to_endpoint(core_id, NocPacketEvent*)`（由 `snn/synapse/route/SpikePacketBridge` 解码并递送到 CoreShell/workload；`MultiCorePE` 仅装配回调）
...
- **禁止**：在 NoC 域内做 fanout 选择或路由表构建（应由 `snn/synapse/route` 负责）。
- **禁止**：在 NoC 域内出现权重/BCSR 语义（应由 `snn/synapse/weights` 与 `snn/synapse/route` 负责）。
