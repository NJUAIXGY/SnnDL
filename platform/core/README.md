# platform/core/（CoreShell：通用子核壳）

本目录存放 **SnnPESubComponent（CoreShell）** 及其内部实现。CoreShell 的定位是：**每个 PE 内的“通用控制壳”**，只负责把 SST 的时钟/packet/统计汇聚接到可插拔 workload 上；任何业务语义（SNN/Step/GAS/BCSR 或非 SNN stream）都应落在 `workloads/*` 中。
...
- `platform/core/*.h/*.cc` **不出现** `StandardMem::`（包括 include `stdMem.h`）。StandardMem glue 必须隔离在 `snn/synapse/stdmem/`。
...
- （隔离文件）`snn/synapse/stdmem/SnnPESubComponent_mem.cc`
  - StandardMem glue 实现文件（刻意不放在 `platform/core/`）。
...
- `workloads/snn`：SNN 业务主链路（输入队列/weights/route/comm/GAS window），并在内部调用 `snn/synapse/*`、`snn/stimulus/*`。
- `workloads/stream`：纯通信/纯内存 streaming（read-after-write verify），不依赖 synapse/stimulus/SpikeEvent。

---

## 扩展指南（新增 workload / compute core）

- 新增 workload：在 `workloads/` 增加实现并注册到 `api/CoreWorkloadFactory.h`，通过 `workload_impl=<name>`（或环境变量 `SNNDL_WORKLOAD_IMPL`）选择。
- 新增 compute core（SNN 内多模型/多范式）：在 `snn/compute/` 实现新的 `ISnnComputeCore`，并由 `workload=snn` 在其内部按参数选择与装配。
