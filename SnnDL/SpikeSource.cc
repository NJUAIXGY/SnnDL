...
SpikeSource::SpikeSource(ComponentId_t id, Params& params) : Component(id) {
    ...
    output->verbose(CALL_INFO, 2, 0, 
        "数据集参数: path=%s, format=%s, time_scale=%.3f, offset=%u, max_events=%u, event_weight=%.3f\n",
        dataset_path.c_str(), dataset_format.c_str(), time_scale, neuron_offset, max_events, event_weight);
