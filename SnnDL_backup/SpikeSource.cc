    ...
    if (segmented_release_) {
        ...
        if (window_open_) {
            ...
            while (!spike_queue.empty() && spike_queue.top().timestamp <= current_sim_time) {
                ...
                spike_event->setDestinationNeuron(spike_data.neuron_id);
                spike_event->setWeight(event_weight);
                if (neurons_per_pe > 0) {
                    uint32_t dest_node = spike_data.neuron_id / neurons_per_pe;
                    spike_event->setDestinationNode(dest_node);
                }
                spike_output_link->send(spike_event);
                events_sent_count++;
                output->verbose(CALL_INFO, 4, 0, "[slice %u] 发送脉冲: 神经元%u, 时间%" PRIu64 "\n", current_slice_idx_, spike_data.neuron_id, spike_data.timestamp);
                spike_queue.pop();
            }
...
bool SpikeSource::loadTextFormat(const std::string& file_path) {
    ...
    while (std::getline(file, line) && (max_events == 0 || events_count < max_events)) {
        ...
        // 应用神经元ID偏移和时间缩放
        uint32_t adjusted_neuron_id = neuron_id + neuron_offset;
